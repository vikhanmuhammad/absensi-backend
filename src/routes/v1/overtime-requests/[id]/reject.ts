  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updated = await db.$transaction(async (tx) => {
        const result = await tx.overtimeRequest.update({ where: { id }, data: { status: 'DITOLAK' } });
        ...
        await notifyUser(tx, {
          ...
          pesan: `Pengajuan lembur individual Anda ditolak.${catatan ? ` Catatan: ${catatan}` : ''}`,
      ...
      return apiOk(res, updated, 'Pengajuan lembur ditolak');
    } catch (error) {
